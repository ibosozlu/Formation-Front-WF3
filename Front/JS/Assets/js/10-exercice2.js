...
var email1 = prompt("Quel est votre email ?");
var mdp1 = prompt("Quel est votre mot de passe ?");


var i = 0;
var z = BaseDeDonnees.length - 1;

function Verification(email1, mdp1){ 
while( (BaseDeDonnees[i].email != email1 || BaseDeDonnees[i].mdp != mdp1) && (i <= z ) ){
    i++;
}
if (i <= z ){
    return true;}
...
if(Verification(email1, mdp1)){
    document.write("<p> Bievenue " + BaseDeDonnees[i]['prenom'] + " " +BaseDeDonnees[i]['nom'] )
}
else{
    alert("Mauvais mdp ou email.");
}
